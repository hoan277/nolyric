...
<div class="simp-display">
    <div class="simp-album w-full flex-wrap">
        <!--<div class="simp-cover"><i class="fa fa-music fa-3x"></i></div>-->
        <div class="simp-info">
            <div class="simp-title">No audio</div>
            <div class="simp-artist">...</div>
        </div>
    </div>
</div>
<div class="simp-controls flex-wrap flex-align">
    ...
    <div class="simp-plauseward flex flex-align">
        <button type="button" class="simp-prev" disabled><i class="simp-prev fa-solid fa-backward"></i></button>
        <button type="button" class="simp-plause" disabled><i class="simp-plause fa-solid fa-play"></i></button>
        <button type="button" class="simp-next" disabled><i class="simp-next fa-solid fa-forward"></i></button>
    </div>
    <div class="simp-others flex flex-align">
        <button type="button" class="" title="Auto Play"><i class="fa-brands fa-autoprefixer"></i></button>
        <button type="button" class="" title="Random"><i class="simp-random fa-solid fa-shuffle"></i></button>
        <!-- <div class="simp-shide">
            <button type="button" class="" title="Show/Hide Album"><i class="simp-shide-top fa-solid fa-caret-up"></i></button>
            <button type="button" class="" title="Show/Hide Playlist"><i class="simp-shide-bottom fa-solid fa-caret-down"></i></button>
        </div> -->
    </div>
    <div class="simp-volume flex flex-align">
        <button type="button" class=""><i class="simp-mute fa-solid fa-volume-high"></i></button>
        <input id="rangeVolume" class="simp-v-slider" type="range" min="0" max="100" value="100" />
    </div>
</div>
`;
...
function simp_changeAudio(elem) {
    ...
    $('button.simp-plause').removeClass('fa-pause fa-play');
}
...
function simp_startScript() {
    ...
    simp_auto_load = simp_config.auto_load || false; //auto load audio file
    ...
    simp_source.forEach(function (item, index) {
        ...
        item.addEventListener('click', function () {
            simp_audio.removeEventListener('timeupdate', simp_initTime);
            simp_a_index = index;
            let simpSrc = this.querySelector('.simp-source');
            // debugger
            simp_changeAudio(simpSrc);
            simp_setAlbum(simp_a_index);
        });
    });
    ...
    simp_volume.addEventListener('click', function (e) {
        ...
        if (eles.contains('simp-mute')) {
            if (eles.contains('fa-volume-up')) {
                ...
                eles.add('fa-volume-mute');
                simp_v_slider.value = 0;
            } else {
                eles.remove('fa-volume-mute');
            ...
            if (simp_v_num != 0) {
                simp_controls.querySelector('.simp-mute').classList.remove('fa-volume-mute');
